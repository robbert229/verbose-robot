...
public class SchedulePatternMatcherTest {
    ...
    public void test(){
        ...
        Assert.assertTrue("Returned command not Schedule",c instanceof CommandMetaDoSchedule);
        CommandMetaDoSchedule command = (CommandMetaDoSchedule)c;
        Assert.assertTrue("Scheduled command not CommandMetaViewDestroy", command.getCommand() instanceof CommandMetaViewDestroy);
        Assert.assertTrue("Scheduled command has invalid parameter", ((CommandMetaViewDestroy)command.getCommand()).getID().equals("FOO"));
    }
